...
def dir_server(
    ...
    server.listen(5)
    print(colored(f"[DIR] Server listening on {address}:{port}", "blue"))

    def handle_client(client_socket: socket.socket, addr: Tuple[str, int]):
        print(colored(f"[DIR] Client connected: {addr}", "blue"))
        ...
        try:
            while True:
                ...
                if not data:
                    print(colored("[DIR] Client disconnected", "red"))
                    break
                ...
                while buffer:
                    ...
                    if flap is None:
                        print(
                            colored(
                                f"[DIR] Partial FLAP received ({len(buffer)} bytes), waiting for more data",
                                "yellow",
                            )
                        )
                        break
                    ...
                    if (
                        ...
                        if flap.get("payload"):
                            # Visualize SNAC
                            print("[DIR] " + parser.visualize(flap["payload"]))
                            ...
                            print(colored("[DIR] Invalid SNAC payload", "red"))

                    else:
                        print(
                            colored(
                                f"[DIR] Unhandled FLAP channel: {flap['channel']:02x}", "red"
                            )
                        )

        except (socket.error, BrokenPipeError) as e:
            print(colored(f"[DIR] Client disconnected with error: {e}", "red"))
        finally:
            client_socket.close()
    ...
    try:
        ...
        print(colored("[DIR] Server shutting down", "blue"))
        server.close()
