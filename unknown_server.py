...
def unk_server(
    ...
    server.listen(5)
    print(colored(f"[UNK] Server listening on {address}:{port}", "blue"))

    def handle_client(client_socket: socket.socket, addr: Tuple[str, int]):
        print(colored(f"[UNK] Client connected: {addr}", "blue"))
        ...
        try:
            while True:
                ...
                if not data:
                    print(colored("[UNK] Client disconnected", "red"))
                    break
                ...
                while buffer:
                    ...
                    if flap is None:
                        print(
                            colored(
                                f"[UNK] Partial FLAP received ({len(buffer)} bytes), waiting for more data",
                                "yellow",
                            )
                        )
                        break
                    ...
                    if (
                        ...
                        if flap.get("payload"):
                            # Visualize SNAC
                            print("[UNK] " + parser.visualize(flap["payload"]))
                            ...
                            print(colored("[UNK] Invalid SNAC payload", "red"))

                    else:
                        print(
                            colored(
                                f"[UNK] Unhandled FLAP channel: {flap['channel']:02x}", "red"
                            )
                        )

        except (socket.error, BrokenPipeError) as e:
            print(colored(f"[UNK] Client disconnected with error: {e}", "red"))
        finally:
            client_socket.close()
    ...
    try:
        ...
        print(colored("[UNK] Server shutting down", "blue"))
        server.close()
