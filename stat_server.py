...
def stat_server(
    ...
    server.listen(5)
    print(colored(f"[STAT] Server listening on {address}:{port}", "blue"))

    def handle_client(client_socket: socket.socket, addr: Tuple[str, int]):
        print(colored(f"[STAT] Client connected: {addr}", "blue"))
        ...
        try:
            while True:
                ...
                if not data:
                    print(colored("[STAT] Client disconnected", "red"))
                    break
                ...
                while buffer:
                    ...
                    if flap is None:
                        print(
                            colored(
                                f"[STAT] Partial FLAP received ({len(buffer)} bytes), waiting for more data",
                                "yellow",
                            )
                        )
                        break
                    ...
                    if (
                        ...
                        if flap.get("payload"):
                            # Visualize SNAC
                            print("[STAT] " + parser.visualize(flap["payload"]))
